...
class NestAankondigingenManager extends BaseModule {
    constructor() {
        ...
        // Voor Tom Select van honden - alleen laden wanneer nodig
        this.allDogs = [];
        ...
        this.translations = {
            nl: {
                ...
                searchDogs: "Zoek een reu of teef door naam of kennelnaam te typen...",
                searchMaleDogs: "Zoek een reu door naam of kennelnaam te typen...",
                searchFemaleDogs: "Zoek een teef door naam of kennelnaam te typen...",
            ...
            en: {
                ...
                searchDogs: "Search for a male or female dog by name or kennel name...",
                searchMaleDogs: "Search for a male dog by name or kennel name...",
                searchFemaleDogs: "Search for a female dog by name or kennel name...",
            ...
            de: {
                ...
                searchDogs: "Suchen Sie einen Rüden oder eine Hündin nach Namen oder Zwingername...",
                searchMaleDogs: "Suchen Sie einen Rüden nach Namen oder Zwingername...",
                searchFemaleDogs: "Suchen Sie eine Hündin nach Namen oder Zwingername...",
                            ...
                            <form id="nestAnnouncementForm">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control" id="father" placeholder="${this.t('typeToSearch')}" ${mode === 'edit' ? 'disabled' : ''}>
                                        ...
                                        <small class="text-muted d-block mt-2">${this.t('typeToSearch')}</small>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="mother" class="form-label">${this.t('mother')}</label>
                                        <select class="form-control" id="mother" placeholder="${this.t('typeToSearch')}" ${mode === 'edit' ? 'disabled' : ''}>
                                            <option value="">${this.t('typeToSearch')}</option>
                                        </select>
    ...
    /**
     * Maak Tom Select voor vader (reuen)
     */
    async initFatherTomSelect(initialValue = null) {
        ...
        const selectElement = document.getElementById('father');
        ...
        const tomSelect = new TomSelect(selectElement, {
            ...
            maxOptions: 100,
            maxItems: 1,
            placeholder: this.t('searchMaleDogs'),
            loadThrottle: 300,
            preload: false,
            load: (query, callback) => {
                if (query.length < 2) {
                ...
                this.searchTimeout = setTimeout(async () => {
                    ...
                    // Sla de gevonden honden op in allDogs en allHonden voor later gebruik
        ...
        if (initialValue) {
            // Als er een initiële waarde is, laad dan de hond en voeg toe
            const supabase = this.getSupabase();
            if (supabase && initialValue) {
                const { data: hond } = await supabase
                    .from('honden')
                    .select('*')
                    .eq('id', initialValue)
                    .single();
                ...
                if (hond) {
                    // Voeg toe aan allDogs en allHonden
                    if (!this.allDogs.find(d => d.id === hond.id)) {
                        this.allDogs.push(hond);
                        this.allHonden.push(hond);
                    }
    ...
    /**
     * Maak Tom Select voor moeder (teven)
     */
    async initMotherTomSelect(initialValue = null) {
        ...
        const selectElement = document.getElementById('mother');
        if (!selectElement) return null;
        ...
        const tomSelect = new TomSelect(selectElement, {
            ...
            maxOptions: 100,
            maxItems: 1,
            placeholder: this.t('searchFemaleDogs'),
            loadThrottle: 300,
            preload: false,
            load: (query, callback) => {
                if (query.length < 2) {
    ...
    /**
     * Haal honden op basis van IDs uit de database en voeg ze toe aan allDogs
     */
    async ensureDogsLoaded(dogIds) {
        if (!dogIds || dogIds.length === 0) return;
        
        const idsToLoad = dogIds.filter(id => !this.allDogs.find(d => d.id === id));
        
        if (idsToLoad.length === 0) return;
        
        console.log(`📦 ${idsToLoad.length} honden laden vanuit database...`);
        ...
        try {
            ...
            console.log(`✅ ${idsToLoad.length} honden toegevoegd aan allDogs`);
        } catch (error) {
            console.error('❌ Fout bij laden honden:', error);
        }
    }
    ...
    async showPuppyPedigree(announcement) {
        try {
            ...
            // Zorg dat de benodigde honden geladen zijn
            await this.ensureDogsLoaded([announcement.vader_id, announcement.moeder_id]);
            
            // Haal vader en moeder op uit allDogs
    ...
    async populateEditForm(announcement) {
        try {
            ...
            // Initialiseer Tom Select voor vader en moeder met de bestaande waarden
            if (announcement.vader_id) {
                await this.initFatherTomSelect(announcement.vader_id);
            } else {
                await this.initFatherTomSelect();
            }
            
            if (announcement.moeder_id) {
                await this.initMotherTomSelect(announcement.moeder_id);
            } else {
                await this.initMotherTomSelect();
            }
    ...
    async saveAnnouncement() {
        ...
        // Haal ID's op van Tom Select
        if (fatherSelect && fatherSelect.tomselect) {
            vaderId = fatherSelect.tomselect.getValue();
        } else {
            vaderId = vaderIdInput?.value;
        }
        
        if (motherSelect && motherSelect.tomselect) {
            moederId = motherSelect.tomselect.getValue();
        } else {
            moederId = moederIdInput?.value;
        }
    ...
    async loadAnnouncements(page = 1) {
        ...
        try {
            ...
            // Verzamel alle unieke hond IDs uit de announcements
            const dogIds = [];
            announcements.forEach(ann => {
                if (ann.vader_id && !dogIds.includes(ann.vader_id)) dogIds.push(ann.vader_id);
                if (ann.moeder_id && !dogIds.includes(ann.moeder_id)) dogIds.push(ann.moeder_id);
            });
            
            // Zorg dat deze honden geladen zijn
            await this.ensureDogsLoaded(dogIds);
    ...
    async renderOverviewList(announcements, container, total = 0, currentPage = 1) {
        ...
        for (const announcement of announcements) {
            // Gebruik de bestaande allDogs lijst voor vader en moeder
            const vader = this.allDogs.find(d => d.id === announcement.vader_id) || {};
            const moeder = this.allDogs.find(d => d.id === announcement.moeder_id) || {};
...
console.log('📦 NestAankondigingenManager geladen met Tom Select voor reu/teef zoeken, 1 per pagina, paginatie 80%, tekst 70%, nestfoto\'s MET OPMERKINGEN (max 15, 1 per keer met paginatie), STAMBOOMWEERGAVE, COMPACTE NEST INFORMATIE, KOPIEERBARE EMAIL en OPTIMIZED FOTO LADEN (alleen thumbnails)');