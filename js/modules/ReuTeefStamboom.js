 ...
 * MET JUISTE DATABASE INITIALISATIE EN PAGINATIE
 */
...
class ReuTeefCombinatie {
    ...
    async loadAllHonden() {
        try {
            if (this.db && typeof this.db.getHonden === 'function') {
                console.log('🔄 ReuTeefCombinatie: laden honden uit database...');
                ...
                // Laad eerste batch honden
                const honden = await this.db.getHonden();
                
                console.log(`✅ ReuTeefCombinatie: ${honden.length} honden geladen uit database`);
                
                // Zorg dat alle gezondheidsvelden aanwezig zijn
                this.allHonden = honden.map(hond => {
                    return {
                        ...hond,
                ...
                this.loadedAllHonden = true;
                console.log(`✅ ReuTeefCombinatie: ${this.allHonden.length} honden in cache`);
                
            } else {
                console.error('❌ ReuTeefCombinatie: Database niet beschikbaar of getHonden functie ontbreekt');
                this.allHonden = [];
            }
        } catch (error) {
    ...
    async loadMoreHonden() {
        ...
        try {
            console.log(`🔄 ReuTeefCombinatie: laad meer honden, pagina ${this.currentPage}...`);
            
            const moreHonden = await this.db.getHonden({ page: this.currentPage });
            
            if (moreHonden.length === 0) {
                this.hasMoreResults = false;
                console.log('✅ ReuTeefCombinatie: alle honden geladen');
                this.isLoading = false;
                return;
            }
            
            // Voeg honden toe aan array
            const newHonden = moreHonden.map(hond => ({
                ...hond,
                heupdysplasie: hond.heupdysplasie || '',
                elleboogdysplasie: hond.elleboogdysplasie || '',
                patella: hond.patella || '',
            ...
            this.allHonden = [...this.allHonden, ...newHonden];
            
            // Update cache
            newHonden.forEach(hond => {
                this.hondenCache.set(hond.id, hond);
                if (hond.stamboomnr) {
                    this.hondenCache.set(hond.stamboomnr, hond);
                }
            ...
            console.log(`✅ ReuTeefCombinatie: ${newHonden.length} extra honden geladen, totaal: ${this.allHonden.length}`);
            
        } catch (error) {
            console.error('❌ ReuTeefCombinatie: Fout bij laden meer honden:', error);
            this.hasMoreResults = false;
        } finally {
            this.isLoading = false;
        }
    ...
    async loadContent() {
        ...
        // Laad eerst honden data VOORDAT we de UI tonen
        if (!this.loadedAllHonden || this.allHonden.length === 0) {
    ...
    async getHondById(id) {
        ...
        try {
            if (this.db && typeof this.db.getHondById === 'function') {
    ...
    async findHondByNameOrPedigree(name) {
        ...
        try {
            if (this.db && typeof this.db.zoekHonden === 'function') {
...
// Exporteer de klasse naar het window object ZOALS ZOEKREU.JS DOET
